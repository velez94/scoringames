...
// Enhanced RBAC authorization for analytics (read-only)
const checkAnalyticsAccess = async (userId, userEmail, organizationId = null) => {
  // Super admin bypass
  if (userEmail === 'admin@athleon.fitness') {
    return { authorized: true, role: 'super_admin' };
  }

  // For organization-specific analytics, check membership
  if (organizationId) {
    try {
  ...
  // For general analytics, any authenticated user can read
  return { authorized: true, role: 'user' };
};

// Legacy function for backward compatibility
const checkOrgAccess = async (organizationId, userId, email) => {
  const authCheck = await checkAnalyticsAccess(userId, email, organizationId);
  return { hasAccess: authCheck.authorized, role: authCheck.role };
};
  }));
...
exports.handler = async (event) => {
  ...
  try {
    const isSuperAdmin = email === 'admin@athleon.fitness';
    let eventIds = [];
    ...
    const [eventsData, athleteRegistrations, categoriesData, wodsData, scoresData] = await Promise.all([
      // Events
      ...
      // Athlete registrations (get all for these events)
      Promise.all(eventIds.map(eventId =>
    ...
    // Get unique athlete details from registrations
    const uniqueAthleteIds = [...new Set(athleteRegistrations.map(a => a.userId))];
    ...
    return {
      ...
      body: JSON.stringify({
        ...
        wods,
        scores,
        athleteRegistrations, // Include registrations for frontend calculations
        stats
      })
    };
