...
class VersusMode {
  constructor(config) {
    ...
    this.eliminationRules = config.eliminationRules; // Custom rules from organizer
    this.progressiveTournament = config.progressiveTournament; // Existing tournament state
  }
  ...
  schedule(athletes, startTime) {
    ...
    const duration = matches.length * this.wodDuration;
  ...
  recalculateSchedule(existingSchedule, newStartTime) {
    ...
    return existingSchedule.map((athlete, index) => {
      ...
      const matchStartTime = startTimeSlot.addMinutes(matchIndex * this.wodDuration);
  ...
  _generateAthleteSchedule(matches, startTime) {
    ...
    matches.forEach((match, index) => {
      const matchStartTime = startTimeSlot.addMinutes(index * this.wodDuration);
