...
class CompetitionScheduler {
  ...
  async generateSchedule(eventId, config) {
    ...
    const {
      ...
      concurrentMatches = 1, // How many matches run simultaneously in VERSUS mode
      startTime = '08:00',
      timezone = 'UTC',
    ...
    if (competitionMode === 'VERSUS') {
      ...
      for (const category of categories) {
        ...
        for (let heatNumber = 1; heatNumber <= categoryNumberOfHeats; heatNumber++) {
          ...
          // Calculate session duration based on concurrent matches
          const matchDuration = wod.estimatedDuration || 15;
          const totalMatches = matches.length;
          const matchRounds = Math.ceil(totalMatches / concurrentMatches);
          const sessionDuration = matchRounds * matchDuration;
          ...
          const session = {
            ...
            endTime: this.minutesToTime(currentTime + sessionDuration),
            duration: sessionDuration,
            matches,
            athleteCount: athletesForHeat.length,
            concurrentMatches,
            athleteSchedule: this.generateAthleteSchedule(matches, currentTime, matchDuration, timezone, concurrentMatches)
          };
  ...
  generateAthleteSchedule(matches, startTime, duration, timezone, concurrentMatches = 1) {
    const schedule = [];
    matches.forEach((match, index) => {
      const timeSlotIndex = Math.floor(index / concurrentMatches);
      const matchStartTime = startTime + (timeSlotIndex * duration);
